...
Фронтенд находится в [`apps/web`](apps/web) и называется `@canvas/web`. Устройство работы с
API, разбор debounce и очереди сохранений, проверенные сценарии и известные недоработки —
...
```sh
npm run build:web      # проверка типов и сборка apps/web
npm run test:web       # модульные тесты фронтенда
npm run e2e:web        # сквозные сценарии на живом API
```

Перед первым запуском сквозных тестов один раз выполните
`npm run e2e:install -w @canvas/web`, чтобы поставить браузер.
